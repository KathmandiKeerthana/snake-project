...
class SnakeGame:
    ...
    def setup_ui(self):
        ...
        self.canvas=tk.Label(self.root,bg=BG_COLOR,width=GAME_WIDTH,height=GAME_HEIGHT)
        self.canvas.pack()
        self.restart_button=tk.Button(self.root,text="Restart",font=("Arial",12,"Bold"),bg="#607D8b",fg="white",command=self.restart_game)
    ...
    def game_loop(self):
        ...
        if[x,y]==self.food_coords:
            ...
            self.canvas.delete(self.sanke_squares[-1])
            self.snake-squares.pop()
            self.snake_coords()
        self.after_id=self.root.after(SPEED,self.game_loop)
    def change_direction(self,now_dir):
